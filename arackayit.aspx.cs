...
public partial class arackayit : System.Web.UI.Page
{
            ...
            else
            {
                ...
                if (vtislemler.sql(Convert.ToString(Session["Sira_No"].ToString())) != -1 && vtislemler.sql(Convert.ToString(Session["Yonetici_Adi"].ToString())) != -1)
                {
                    ...
                    if (dtyonetici.Rows.Count > 0)
                    {
                        ...
                        string strdataaraclar = "<table class=\"table table-bordered table-responsive\"><thead><tr><th class=\"color-white bg-black\">MARKA</th><th class=\"color-white bg-black\">MODEL</th><th class=\"color-white bg-black\">YIL</th><th class=\"color-white bg-black\">FİYAT</th></tr></thead><tbody>";
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            strdataaraclar += "<tr><td>"+dt.Rows[i]["Marka"].ToString()+ "</td><td>" + dt.Rows[i]["Model"].ToString() + "</td><td>" + dt.Rows[i]["Yıl"].ToString() + "</td><td>" + dt.Rows[i]["Fiyat"].ToString() + "</td></tr>";
                        }
    ...
    protected void Timer1_Tick(object sender, EventArgs e)
    {
        ...
        Response.Redirect("arackayit.aspx");
